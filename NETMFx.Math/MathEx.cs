...
namespace NETMFx.Math
{
    ...
    /// Changes by Ian Lee:
    /// 
    /// Re-wrote Atan2 function to return values consistent with expected Atan2 values.
    /// Expanded precision of Pi.
    /// Made Pow(float x, int y) public.
    /// </summary>
    public static class MathEx
    {
        ...
        /// </summary>
        public static readonly float Pi = 3.1415926535897932384626433832795F;       //3.1415926535897931F;
...
/*
        ...
        {
            return 2*Atans((Sqrt(Pow(x, 2) + Pow(y, 2)) - x)/y);
        }
*/

        public static float Atan2(float y, float x)
        {
            if (x > 0.0F) return Atans(y/x);
            if (x < 0.0F) return ((y >= 0.0F) ? Pi : (-Pi)) + Atans(y/x);
            if (Abs(y - 0F) < 1E-10) return 0;       // undefined
        ...
        public static float Cosh(float x)
        {
            ...
            if (x <= (Ln2 / 2))
            {
                return (1 + (Pow((Exp(x) - 1), 2) / (2 * Exp(x))));
            }
            if (x <= 22F)
            {
        ...
        public static float Log(float x, float newBase)
        {
            ...
            if (Abs(x - 0.0F) < 1E-10) return (float)double.NegativeInfinity;
